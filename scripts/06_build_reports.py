...
1月作为离线SOC初始化：2025-01-01 00:00从6000 kWh开始，不设每日首尾相等；1月费用不计入问题二。三种策略共同以 `{manifest['common_feb1_soc_kwh']:.3f}` kWh进入2月1日，之后分别连续传递实际SOC。B1用验证胜出的点预测做确定性计划；B3为20场景、共享日前G的两阶段随机LP；B4增加标准CVaR线性化，alpha=0.95，lambda由1月22-31日数据选为 `{manifest['chosen_lambda']}`。
