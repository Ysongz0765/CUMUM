...
def main():
    ...
    sources43 = pd.read_csv(OUT / "q4_3_scenario_sources.csv", parse_dates=["source_date", "target_date", "training_cutoff", "decision_time", "source_observation_cutoff", "target_observation_cutoff"])
    ...
    expected_source_cutoff = sources43.source_date + pd.to_timedelta(sources43.issue_hour, unit="h")
    evidence["q4_3_historical_anchor_cutoffs_exact"] = bool(
        sources43.boundary_method.eq("observed_anchor").all()
        and (sources43.source_observation_cutoff == expected_source_cutoff).all()
        and (sources43.target_observation_cutoff == sources43.decision_time).all()
    )
