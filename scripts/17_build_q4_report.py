...
问题四使用附件4逐日、逐10分钟时段电价，附件2提供历史及实际负荷/光伏，4-3另外使用附件3按00:00、06:00、12:00、18:00发布的光伏预报。4-3发布时点以截至该时点已经结束的最后一个10分钟实际光伏时段作为左锚点，与下一原始整点预报线性衔接，不读取发布后实际值。主模型假设一天144个附件4价格在当天00:00已知；这是沿用v3.1的建模假设，题面只提供全年价格数据，并未明确承诺日前公布。本轮没有增加价格预测模型。价格单位直接为元/kWh，不除以6；只有kW功率转10分钟电量时除以6。
...
10个校准日的经验日CVaR95等于最大日值，不是稳定的尾部风险估计；10个等权规划场景且alpha=0.95时，场景紧急费用CVaR同样等于最坏场景费用，不能描述为稳定估计了全年尾部风险。
